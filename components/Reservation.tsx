  ...
  return (
    <section id="foglalas" className="section-padding relative noise-overlay overflow-hidden">
      ...
      <div className="container-main relative z-10">
        <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} className="text-center mb-10 md:mb-14">
          ...
          <motion.h2 variants={staggerItem} className="font-heading font-semibold mb-4 text-glow" style={{ fontSize: 'clamp(2rem, 4vw, 3.5rem)' }}>Foglalj asztalt</motion.h2>
        ...
        <motion.form variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }}
          onSubmit={(e) => e.preventDefault()} className="max-w-3xl mx-auto card-3d p-8 md:p-12 !rounded-3xl">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6 mb-10">
            {[
              ...
              <div key={f.id}>
                <label htmlFor={f.id} className="block text-xs tracking-[0.15em] uppercase text-text-muted mb-2.5 font-medium">{f.label}</label>
            ...
            <div>
              <label htmlFor="guests" className="block text-xs tracking-[0.15em] uppercase text-text-muted mb-2.5 font-medium">Vendégszám</label>
            ...
            <div>
              <label htmlFor="date" className="block text-xs tracking-[0.15em] uppercase text-text-muted mb-2.5 font-medium">Dátum</label>
              <input type="date" id="date" name="date" value={form.date} onChange={set} required className={`${inp} cursor-pointer`} />
            </div>
            <div>
              <label htmlFor="time" className="block text-xs tracking-[0.15em] uppercase text-text-muted mb-2.5 font-medium">Időpont</label>
          ...
          <button type="submit" className="btn-primary w-full md:w-auto md:min-w-[240px] md:mx-auto md:block !py-4 !text-sm">Asztal foglalása</button>
        </motion.form>
      </div>
    </section>
  );
